...
## 主要功能
- 主题切换与跟随系统
  - 本版本优化了主题切换：删除单独的“跟随系统”开关，改为一体式主题切换按钮（顶部太阳/月亮图标）。默认行为为：主题会跟随系统自动调整，用户仍可点击主题按钮在“浅色/深色”之间手动切换（`index.html:28–36`）。
...
- 常用操作
  - 切换主题：点击顶部太阳/月亮按钮；或启用“跟随系统”。
   - 切换主题：
     - 默认：应用会跟随操作系统主题并自动调整（默认行为）。
     - 手动切换到浅色模式：点击顶部的主题切换按钮，直到显示为“浅色/太阳”图标。
     - 手动切换到深色模式：点击顶部的主题切换按钮，直到显示为“深色/月亮”图标。
